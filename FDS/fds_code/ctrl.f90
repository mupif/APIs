...
REAL(EB), INTENT(IN) :: T,DT
REAL(EB) :: RAMP_VALUE,T_CHANGE,RAMP_INPUT,PID_VALUE,R_I
INTEGER :: NC,COUNTER
...
CONTROL_SELECT: SELECT CASE (CF%CONTROL_INDEX)
  ...
  CASE (OR_GATE)
   ...
   CASE (DEADBAND_TD)
       ...
       R_I=RAMP_INPUT
       IF (CF%ON_BOUND > 0) THEN
          IF (DV%SMOOTHED_VALUE > EVALUATE_RAMP(R_I,0._EB,CF%SPI2) .AND. (CF%CURRENT_STATE .EQV. CF%INITIAL_STATE)) THEN
             STATE2 = .TRUE.
          ELSEIF(DV%SMOOTHED_VALUE < EVALUATE_RAMP(R_I,0._EB,CF%SPI1) .AND. (CF%CURRENT_STATE .NEQV. CF%INITIAL_STATE)) THEN
             STATE2 = .FALSE.
          ELSEIF(DV%SMOOTHED_VALUE >= EVALUATE_RAMP(R_I,0._EB,CF%SPI1) .AND. (CF%CURRENT_STATE .NEQV. CF%INITIAL_STATE)) THEN
             STATE2 = .TRUE.
          ENDIF
       ELSE
          IF (DV%SMOOTHED_VALUE < EVALUATE_RAMP(R_I,0._EB,CF%SPI1) .AND. (CF%CURRENT_STATE .EQV. CF%INITIAL_STATE)) THEN
             STATE2 = .TRUE.
          ELSEIF(DV%SMOOTHED_VALUE > EVALUATE_RAMP(R_I,0._EB,CF%SPI2) .AND. (CF%CURRENT_STATE .NEQV. CF%INITIAL_STATE)) THEN
             STATE2 = .FALSE.
          ELSEIF(DV%SMOOTHED_VALUE <= EVALUATE_RAMP(R_I,0._EB,CF%SPI2) .AND. (CF%CURRENT_STATE .NEQV. CF%INITIAL_STATE)) THEN
             STATE2 = .TRUE.
          ENDIF
