...
PUBLIC ASSIGN_FILE_NAMES,INITIALIZE_GLOBAL_DUMPS,INITIALIZE_MESH_DUMPS,WRITE_STATUS_FILES, &
       ...
       nnodes_x,nnodes_y,nnodes_z,get_temp,get_temp_field
...
integer function nnodes_x(meshID) bind(C, name="nnodes_x")
  integer(c_int), intent(in) :: meshID
  nnodes_x=MESHES(meshID)%IBP1
end function nnodes_x
integer function nnodes_y(meshID) bind(C, name="nnodes_y")
  integer(c_int), intent(in) :: meshID
  nnodes_y=MESHES(meshID)%JBP1
end function nnodes_y
integer function nnodes_z(meshID) bind(C, name="nnodes_z")
  integer(c_int), intent(in) :: meshID
  nnodes_z=MESHES(meshID)%KBP1
end function nnodes_z

integer(c_int) function get_temp(arg1,i,j,k,meshID) bind(C, name="give_teplotu")
  real(c_double), intent(inout) :: arg1
  integer(c_int), intent(in) :: i,j,k
  integer(c_int), intent(in) :: meshID
  arg1 = (MESHES(meshID)%TMP(i,j,k) - TMPM)
  get_temp = 1
end function get_temp

integer(c_int) function get_temp_field(arg_field,meshID) bind(C, name="get_temp_field")
  integer(c_int), intent(in) :: meshID
  real(c_double), intent(inout) :: arg_field(MESHES(meshID)%IBP1*MESHES(meshID)%JBP1*MESHES(meshID)%KBP1)
  integer :: i,j,k,idx
  TYPE (MESH_TYPE), POINTER :: meshpointer
  meshpointer => MESHES(meshID)
  idx=0
  do k=1,MESHES(meshID)%KBP1
    do j=1,MESHES(meshID)%JBP1
      do i=1,MESHES(meshID)%IBP1
        idx=idx+1
        arg_field(idx) = (MESHES(meshID)%TMP(i,j,k) - TMPM)
      enddo
    enddo
  enddo
  get_temp_field = 1
end function get_temp_field

REAL(EB) function give_me_astg(pointid,astg) bind(C, name="give_me_astg")
    integer(c_int), intent(in) :: pointid
    real(c_double), intent(inout) :: astg
    integer, POINTER :: nulak
    nulak=0
    astg=SOLID_PHASE_OUTPUT(1,23,-1,-1,1,%VAL(0),OPT_LP_INDEX=pointid)
    give_me_astg=astg
end function give_me_astg
